...
The first parameter `$value` of the `Inject` constructor is required and of type mixed. The second parameter `$type` is optional and of type `Celema\Wire\Type` which is a enum. Both are availabe as public instance properties. Every additional argument is avalable via the `meta` property.

```php
--8<-- "inject-instance.php:7"
```

!!! info "Note" In most cases, you will only work directly with an `Inject` instance if you use the Inject type `Type::Callback`. See [below](#celemawiretypecallback).
...
The inject type is passed as second argument to the `Inject` attribute und must be of the data type `Celema\Wire\Type`:

```php
// a valid array
#[Inject('value', Celema\Wire\Type::Literal)]
```

The available types are:

### `Celema\Wire\Type::Literal`
